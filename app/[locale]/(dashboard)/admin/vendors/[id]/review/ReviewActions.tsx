...
export default function ReviewActions({ vendorId, currentStatus, rejectionReason, locale }: ReviewActionsProps) {
        ...
        <div className="bg-white dark:bg-[#111] rounded-[2rem] p-8 border border-gray-100 dark:border-white/5 shadow-xl space-y-6">
            <div className="space-y-1">
                <h3 className="text-sm font-black uppercase tracking-tighter italic flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-blue-500" /> {isFr ? 'Décision Admin' : 'Admin Decision'}
                </h3>
                <p className="text-[10px] text-muted-foreground uppercase font-black tracking-widest">
                    {isFr ? 'Sélectionnez une action pour ce marchand' : 'Select an action for this merchant'}
                </p>
            </div>
            ...
            <div className="space-y-3">
                ...
                >
                    ...
                    {isFr ? 'Approuver le Marchand' : 'Approve Merchant'}
                </Button>
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                        <MessageSquare className="w-4 h-4" /> {isFr ? 'Correction' : 'Correction'}
                    </Button>
                    <Button 
                        variant="outline"
                        onClick={() => {
                    ...
                    >
                        <XCircle className="w-4 h-4" /> {isFr ? 'Rejeter' : 'Reject'}
                    </Button>
                </div>
            </div>
            ...
            {showReasonField && (
                <div className="space-y-4 pt-4 border-t border-gray-100 dark:border-white/5 animate-in slide-in-from-top-4 duration-300">
                    <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                            ...
                            {pendingAction === 'REJECT' 
                                ? (isFr ? 'Raison du Rejet' : 'Reason for Rejection') 
                                : (isFr ? 'Corrections Requises' : 'Required Corrections')
                            }
                        </label>
                        <Textarea 
                            value={reason}
                            ...
                            placeholder={pendingAction === 'REJECT' 
                                ? (isFr ? "Expliquez pourquoi ce vendeur a été rejeté..." : "Explain why this vendor was rejected...") 
                                : (isFr ? "Dites au vendeur ce qui doit être corrigé..." : "Tell the vendor what needs to be fixed...")
                            }
                    ...
                    <div className="flex gap-2">
                        ...
                        >
                            {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : (isFr ? 'Soumettre' : 'Submit Decision')}
                        </Button>
                    </div>
                </div>
            )}
            ...
            {status === 'APPROVED' && !showReasonField && (
                <div className="p-4 bg-green-500/10 border border-green-500/20 rounded-2xl flex gap-3">
                    ...
                    <p className="text-xs text-green-700 font-bold">
                        {isFr ? 'Ce marchand est vérifié et actif.' : 'This merchant is verified and active on the marketplace.'}
                    </p>
                </div>
            )}
        </div>
